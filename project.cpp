...
void decompressor(string& inputFileName) {
    ...
    std::unordered_map<std::string, char> reverseHuffmanCodes; // New map for decoding
    for (size_t i = 0; i < huffmanCodesSize; ++i) {
        ...
        std::string code;
        code.reserve(codeLength); // Reserve memory for the code
        for (int j = 0; j < codeLength; ++j) {
            code += inputFile.get();
        }

        huffmanCodes[character] = code;
        reverseHuffmanCodes[code] = character; // Fill the reverse map
    }
    ...
    // Use a vector to store the encoded bits
    std::string encodedBits;
    encodedBits.reserve(encodedBitsSize); // Reserve memory for encoded bits
    char byte;
    ...
    // Decode the text using the reverse map
    std::string decodedText;
    std::string currentCode;

    for (char bit : encodedBits) {
        currentCode += bit;
        auto it = reverseHuffmanCodes.find(currentCode); // Use the reverse map
        if (it != reverseHuffmanCodes.end()) {
            decodedText += it->second; // Append the character
            currentCode.clear();
        }
    }

    std::ofstream out_file("decompressed.txt");
